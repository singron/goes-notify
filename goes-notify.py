...
def notify_send_email(dates, current_apt, settings, use_gmail=False,
    ...
    try:
        if use_gmail:
            ...
            if not password:
                logging.warning(
                    'Trying to send from gmail, but password was not provided.')
                return
            ...
            server = smtplib.SMTP(settings.get(
                'email_server'), settings.get('email_port'))
            server.ehlo()
            server.starttls()
            server.ehlo()
            if username:
        ...
        message = EMAIL_TEMPLATE % (
            dateshtml, current_apt.strftime('%B %d, %Y'))
...
def notify_osx(msg):
    subprocess.check_output(['osascript', '-e'
                             f'display notification \"{msg}\" with title \"Global Entry Notifier\"'])
...
def notify_sms(settings, dates):
    for avail_apt in dates:
        try:
            ...
            logging.warning(
                'Trying to send SMS, but TwilioRestClient not installed. Try \'pip install twilio\'')
            return
        ...
        try:
            ...
            logging.warning(
                'Trying to send SMS, but one of the required Twilio settings is missing or empty')
            return
...
def main(settings):
    try:
        ...
        hash = hashlib.md5((''.join(
            dates) + current_apt.strftime('%B %d, %Y @ %I:%M%p')).encode('utf-8')).hexdigest()
    ...
    msg = 'Found new appointment(s) in location %s on %s (current is on %s)!' % (settings.get(
        "enrollment_location_id"), dates[0], current_apt.strftime('%B %d, %Y @ %I:%M%p'))
    logging.info(msg + (' Sending email.' if not settings.get('no_email')
                 else ' Not sending email.'))
...
def _check_settings(config):
    ...
    if config.get('use_gmail') and not config.get('gmail_password'):
        raise ValueError(
            'gmail_password not found in config but is required when running with use_gmail option')
